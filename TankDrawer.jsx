...
function TankRow({tank,index,total,isActive,onSelect,onMove,onRename,onDelete,onSpeed}){
    ...
    <div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 6px",borderRadius:8,marginBottom:4,cursor:"pointer",background:isActive?"rgba(77,150,255,.06)":"transparent",border:`1px solid ${isActive?"rgba(77,150,255,.15)":"transparent"}`}}>
      ...
      <div onClick={onSelect} style={{flex:1,minWidth:0}}>
        {editing?(<input autoFocus value={name} onChange={e=>setName(e.target.value)}
          ...
          style={{width:"100%",padding:"2px 4px",background:"rgba(0,0,0,.3)",border:"1px solid rgba(77,150,255,.3)",borderRadius:4,color:"#d0d8e4",fontSize:11,fontFamily:"inherit",boxSizing:"border-box"}}/>
      ...
      <button onClick={e=>{e.stopPropagation();setEditing(true);setName(tank.name);}} style={{...tinyBtn,fontSize:9}}>{"\u270F\uFE0F"}</button>
      <button onClick={e=>{e.stopPropagation();onDelete();}} style={{...tinyBtn,color:"#FF4757",fontSize:9}}>{"\uD83D\uDDD1\uFE0F"}</button>
    </div>
  );
}

export default function TankDrawer({ drawer, setDrawer, tanks, activeId, setActiveId, moveTank, renameTank, setDelModal, cycleSpeed, addTank, setJoinModal, MAX_TANKS }) {
  if(!drawer)return null;
  return(<>
    <div onClick={()=>setDrawer(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.5)",zIndex:38}}/>
    <div style={{position:"absolute",top:0,left:0,bottom:0,width:"min(300px, 80vw)",background:"rgba(8,12,24,.98)",zIndex:40,display:"flex",flexDirection:"column",boxShadow:"4px 0 30px rgba(0,0,0,.5)",animation:"slideRight .2s ease-out",backdropFilter:"blur(10px)"}}>
      ...
      {tanks.length<MAX_TANKS&&(<div style={{padding:"12px 16px",borderTop:"1px solid rgba(255,255,255,.04)"}}>
