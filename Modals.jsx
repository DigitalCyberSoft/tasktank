...
export function DeleteModal({ delModal, setDelModal, tanks, confirmDelete }) {
  ...
  return(<><div onClick={()=>setDelModal(null)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.6)",zIndex:48}}/>
    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(320px, 88vw)",background:"var(--modal,rgba(12,8,8,.97))",border:"1px solid rgba(255,70,70,.15)",borderRadius:14,padding:20,zIndex:50,animation:"fadeIn .2s ease-out",boxShadow:"0 12px 40px rgba(0,0,0,.6)"}}>
      ...
      <div style={{fontSize:10,textAlign:"center",opacity:.5,marginBottom:14,lineHeight:1.6}}>
        {isEmpty?<>Delete empty tank <b style={{color:"var(--tx,#eee)"}}>{tank.name}</b>?</>
        :<>Permanently destroy <b style={{color:"var(--tx,#eee)"}}>{tank.name}</b> and {(tank.fishes||[]).length} fish. Type the name to confirm.</>}</div>
      {!isEmpty&&<input autoFocus value={delModal.input} onChange={e=>setDelModal(p=>({...p,input:e.target.value}))} placeholder={tank.name}
        style={{width:"100%",padding:"10px 12px",background:"var(--inp,rgba(255,255,255,.04))",border:"1px solid rgba(255,70,70,.15)",borderRadius:8,color:"var(--tx,#d0d8e4)",fontSize:12,fontFamily:"inherit",boxSizing:"border-box",marginBottom:14}}/>}
      <div style={{display:"flex",gap:8}}>
        <button onClick={()=>setDelModal(null)} style={{flex:1,padding:"10px 0",background:"var(--inp,rgba(255,255,255,.04))",border:"1px solid var(--brd,rgba(255,255,255,.08))",borderRadius:8,color:"var(--tx2,#a8b4c0)",cursor:"pointer",fontSize:11,fontFamily:"inherit"}}>Cancel</button>
...
export function NewTankModal({ newTankModal, setNewTankModal, onCreateTank }) {
  ...
  return(<><div onClick={close} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.6)",zIndex:48}}/>
    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(320px, 88vw)",background:"var(--modal,rgba(8,14,28,.97))",border:"1px solid rgba(77,150,255,.15)",borderRadius:14,padding:20,zIndex:50,animation:"fadeIn .2s ease-out",boxShadow:"0 12px 40px rgba(0,0,0,.6)"}}>
      ...
      <input autoFocus value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")doCreate();}}
        ...
        style={{width:"100%",padding:"10px 12px",background:"var(--inp,rgba(255,255,255,.04))",border:"1px solid rgba(77,150,255,.15)",borderRadius:8,color:"var(--tx,#d0d8e4)",fontSize:12,fontFamily:"inherit",boxSizing:"border-box",marginBottom:14}}/>
      <div style={{display:"flex",gap:8}}>
        <button onClick={close} style={{flex:1,padding:"10px 0",background:"var(--inp,rgba(255,255,255,.04))",border:"1px solid var(--brd,rgba(255,255,255,.08))",borderRadius:8,color:"var(--tx2,#a8b4c0)",cursor:"pointer",fontSize:11,fontFamily:"inherit"}}>Cancel</button>
...
// ══════════════════════════════════════════════════════════════
export function ListViewOverlay({ listView, setListView, actTank, catchFish, toggleFishComplete, isReadonly }) {
  ...
  const toggleBtn=(f)=>(<div onClick={e=>{e.stopPropagation();if(!isReadonly)toggleFishComplete(actTank.id,f.id);}}
    style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${f.completed?"#2ED573":"rgba(255,255,255,.2)"}`,
      ...
      flexShrink:0,cursor:isReadonly?"default":"pointer",transition:"all .15s",opacity:isReadonly?.5:1}}>
    {f.completed&&<span style={{color:"#fff",fontSize:12,fontWeight:700,lineHeight:1}}>{"\u2713"}</span>}
  </div>);
  return(
    <div style={{position:"absolute",inset:0,zIndex:180,background:"var(--surf,rgba(8,12,24,.98))",backdropFilter:"blur(8px)",display:"flex",flexDirection:"column",animation:"fadeIn .2s ease-out"}}>
      <div style={{padding:"12px 16px",borderBottom:"1px solid var(--brd,rgba(255,255,255,.06))",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        <div style={{fontSize:9,opacity:.3,display:"flex",gap:8,alignItems:"center"}}>
          ...
          <div style={{position:"relative"}}>
            ...
            {copyMenu&&(<div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:"var(--modal,rgba(12,18,32,.98))",border:"1px solid rgba(77,150,255,.2)",borderRadius:8,padding:8,zIndex:10,minWidth:150,boxShadow:"0 8px 24px rgba(0,0,0,.5)"}}>
              <button onClick={()=>doCopy("plain")} style={{display:"block",width:"100%",padding:"8px 10px",background:"none",border:"none",borderRadius:4,color:"var(--tx,#d0d8e4)",fontSize:10,fontFamily:"inherit",cursor:"pointer",textAlign:"left"}} className="ni">Plain text</button>
              <button onClick={()=>doCopy("rich")} style={{display:"block",width:"100%",padding:"8px 10px",background:"none",border:"none",borderRadius:4,color:"var(--tx,#d0d8e4)",fontSize:10,fontFamily:"inherit",cursor:"pointer",textAlign:"left"}} className="ni">With details</button>
              <div style={{borderTop:"1px solid var(--brd,rgba(255,255,255,.06))",margin:"4px 0"}}/>
      ...
      <div style={{flex:1,overflow:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:16}}>
        ...
        {(()=>{const items=fishes.filter(f=>!f.completed&&(f.importance||"normal")==="normal"&&f.importance!=="critical"&&f.importance!=="important");
          ...
          return(<div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {items.map(f=>(<div key={f.id} onClick={()=>{catchFish(actTank.id,f.id);setListView(false);}} style={{padding:"12px 14px",background:"var(--inp,rgba(255,255,255,.03))",border:"1px solid var(--brd,rgba(255,255,255,.06))",borderRadius:8,cursor:"pointer",transition:"all .15s",display:"flex",alignItems:"center",gap:10}} className="ni">
                {toggleBtn(f)}
                <div style={{flex:1,minWidth:0}}>
      ...
      <div style={{padding:"12px 16px",borderTop:"1px solid var(--brd,rgba(255,255,255,.06))",flexShrink:0}}>
...
export function PurgeOverlay({ nukeId, nukeTank, keepers, toggleK, doPurge, setNukeId, setKeepers }) {
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:200,background:"var(--surf,rgba(6,2,2,.98))",backdropFilter:"blur(8px)",display:"flex",flexDirection:"column",padding:"14px 12px",animation:"fadeIn .2s ease-out"}}>
      <div style={{textAlign:"center",marginBottom:12,flexShrink:0}}>
        ...
        <div style={{fontSize:10,opacity:.6,marginTop:4,letterSpacing:1,color:"var(--tx2,#ccc)"}}>Tap to save. Rest get flushed.</div>
      </div>
      <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column",gap:4}}>
        {(nukeTank.fishes||[]).map(f=>{const saved=keepers.has(f.id);const imp=IMP[f.importance||"normal"];
          return(<div key={f.id} className="ni" onClick={()=>toggleK(f.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:8,cursor:"pointer",background:saved?"rgba(46,213,115,.1)":"rgba(255,71,87,.08)",border:`1.5px solid ${saved?"rgba(46,213,115,.25)":"rgba(255,71,87,.2)"}`,transition:"all .15s"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,fontWeight:saved?600:400,color:saved?"var(--tx,#e8f0e8)":"#aa8888",textDecoration:saved?"none":"line-through",wordBreak:"break-word",lineHeight:1.4}}>{f.task}</div>
      ...
      <div style={{display:"flex",gap:8,marginTop:12,justifyContent:"center",flexShrink:0}}>
        <button onClick={()=>{setNukeId(null);setKeepers(new Set());}} style={{padding:"12px 20px",background:"var(--inp,rgba(255,255,255,.06))",border:"1px solid var(--brd3,rgba(255,255,255,.12))",borderRadius:8,color:"var(--tx,#d0d8e4)",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:500}}>Cancel</button>
    ...
    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(380px, 92vw)",maxHeight:"85vh",background:"var(--modal,rgba(8,14,28,.98))",border:"1px solid rgba(77,150,255,.15)",borderRadius:14,zIndex:50,animation:"fadeIn .2s ease-out",boxShadow:"0 12px 40px rgba(0,0,0,.6)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"12px 18px 18px"}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:14}}>
          {PERM_OPTS.map(p=>(<button key={p.v} onClick={()=>setSharePerm(p.v)} style={{flex:1,padding:"8px 6px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",background:sharePerm===p.v?p.c+"14":"var(--inp,rgba(255,255,255,.02))",border:`1.5px solid ${sharePerm===p.v?p.c+"44":"var(--brd,rgba(255,255,255,.04))"}`,transition:"all .12s"}}>
            <div style={{fontSize:10,fontWeight:sharePerm===p.v?700:400,color:sharePerm===p.v?p.c:"var(--tx3,#778)",letterSpacing:.5}}>{p.l}</div>
            <div style={{fontSize:7,opacity:.35,marginTop:2}}>{p.d}</div></button>))}
        </div></>}

        {/* Recipient name */}
        {isOwner&&<><div style={{fontSize:9,opacity:.35,letterSpacing:1.5,marginBottom:6,fontWeight:600}}>FOR (OPTIONAL)</div>
        ...
        <div style={{fontSize:7,opacity:.25,marginBottom:8,lineHeight:1.5}}>Share this code with another device. They paste it in Join to sync this tank. Code expires in 5 minutes.</div>
        {pairLink&&<button onClick={()=>{try{navigator.clipboard.writeText(pairLink);}catch{const ta=document.createElement("textarea");ta.value=pairLink;ta.style.cssText="position:fixed;left:-9999px";document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);}setLinkFlash(true);setTimeout(()=>setLinkFlash(false),1500);}}
          style={{width:"100%",padding:"10px 0",marginBottom:14,background:linkFlash?"rgba(46,213,115,.12)":"linear-gradient(135deg,rgba(77,150,255,.15),rgba(107,203,119,.15))",border:`1px solid ${linkFlash?"rgba(46,213,115,.3)":"rgba(77,150,255,.2)"}`,borderRadius:8,color:linkFlash?"#2ED573":"#7bb8ff",fontWeight:700,fontSize:11,cursor:"pointer",fontFamily:"inherit",letterSpacing:1,transition:"all .15s"}}>{linkFlash?"\u2713 Link Copied!":"\uD83D\uDD17 Copy Link"}</button>}
        </>}
        ...
        {isOwner&&<div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
          <div style={{width:140,height:140,background:"#fff",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",padding:8,position:"relative",overflow:"hidden"}}>
            ...
            :<div style={{fontSize:9,color:"var(--tx2,#888)",textAlign:"center"}}>{loading?"Generating...":"QR loading..."}</div>}
        ...
        {peers.length===0?(
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:7,opacity:.3,marginTop:1}}>
              ...
              ):(<span style={{color:p.permission==="shared"?"#6BCB77":"#4D96FF",fontWeight:600}}>{p.permission||"shared"}</span>)}
        ...
        {/* Leave tank (non-owners only) */}
        {!isOwner&&leaveTank&&<button onClick={()=>{leaveTank(tank.id);setShareModal(null);}}
          style={{width:"100%",padding:"10px 0",marginTop:14,background:"rgba(255,71,87,.06)",border:"1px solid rgba(255,71,87,.15)",borderRadius:8,color:"#FF4757",fontWeight:700,fontSize:11,cursor:"pointer",fontFamily:"inherit",letterSpacing:1,transition:"all .15s"}}>{"\u21A9"} Leave Tank</button>}

        {/* Device info */}
        <div style={{marginTop:14,padding:"8px 10px",borderRadius:6,background:"var(--inp,rgba(255,255,255,.01))",border:"1px solid var(--brd2,rgba(255,255,255,.025))"}}>
          <div style={{fontSize:8,opacity:.25,letterSpacing:1,marginBottom:3}}>THIS DEVICE</div>
          <div style={{fontSize:8,fontFamily:"monospace",color:"var(--tx3,#556)",wordBreak:"break-all"}}>{DEVICE_ID}</div>
          {tank.ownerId&&tank.ownerId!==DEVICE_ID&&<div style={{fontSize:8,color:"#FFD93D",opacity:.5,marginTop:3}}>You are a {tank.myPermission||(peers.find(p=>p.deviceId===DEVICE_ID)||{permission:"shared"}).permission} member</div>}
        </div>
      </div>
    </div></>);
}
...
export function JoinModal({ joinModal, setJoinModal, parseShareCode, acceptPair, syncStatus, initialCode }) {
  ...
  // Pre-fill from pairing link
  useEffect(()=>{if(initialCode&&joinModal){setJoinCode(initialCode);setJoinError("");}},[initialCode,joinModal]);
  ...
  // Extract pair code from a scanned value (may be URL or raw base64)
  const extractCode=(val)=>{try{const u=new URL(val);return new URLSearchParams(u.search).get("pair")||val;}catch{return val;}};
        ...
        scanTimerRef.current=setInterval(async()=>{
          ...
          try{
            if("BarcodeDetector"in window){
              ...
              if(results.length>0){setJoinCode(extractCode(results[0].rawValue));setJoinError("");stopScanner();return;}
            }
          }catch{}
          // jsQR fallback
          try{
            ...
            if(result){setJoinCode(extractCode(result.data));setJoinError("");stopScanner();return;}
          }catch{}
        },250);
      },100);
  ...
  return(<><div onClick={close} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.6)",zIndex:48}}/>
    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(360px, 90vw)",maxHeight:"85vh",background:"var(--modal,rgba(8,14,28,.98))",border:"1px solid rgba(77,150,255,.15)",borderRadius:14,padding:20,zIndex:50,animation:"fadeIn .2s ease-out",boxShadow:"0 12px 40px rgba(0,0,0,.6)",overflow:"auto"}}>
      ...
      <textarea value={joinCode} onChange={e=>{setJoinCode(e.target.value);setJoinError("");}} placeholder="Paste pairing code here\u2026" rows={3}
        style={{width:"100%",padding:"10px 12px",background:"var(--inp,rgba(255,255,255,.04))",border:"1px solid rgba(77,150,255,.12)",borderRadius:8,color:"var(--tx,#d0d8e4)",fontSize:10,fontFamily:"monospace",resize:"vertical",boxSizing:"border-box"}}/>
      ...
      <div style={{marginTop:14,padding:"8px 10px",borderRadius:6,background:syncStatus==="connected"?"rgba(46,213,115,.04)":"rgba(255,255,255,.01)",border:`1px solid ${syncStatus==="connected"?"rgba(46,213,115,.1)":"rgba(255,255,255,.025)"}`}}>
        <div style={{fontSize:8,display:"flex",alignItems:"center",gap:4}}>
          ...
          <span style={{color:syncStatus==="connected"?"#2ED573":"var(--tx3,#667)",fontWeight:600}}>P2P Sync {syncStatus==="connected"?"active":"offline"}</span>
        </div>
...
export function BulkAddModal({ bulkModal, setBulkModal, actTank, activeId, bulkAddFish: parentBulkAdd, initP }) {
  ...
  return(<><div onClick={close} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.6)",zIndex:48}}/>
    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(420px, 92vw)",maxHeight:"80vh",background:"var(--modal,rgba(8,14,28,.98))",border:"1px solid rgba(77,150,255,.15)",borderRadius:14,zIndex:50,animation:"fadeIn .2s ease-out",boxShadow:"0 12px 40px rgba(0,0,0,.6)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{padding:"14px 18px 10px",borderBottom:"1px solid var(--brd,rgba(255,255,255,.04))",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"12px 18px 18px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <textarea value={bulkText} onChange={e=>setBulkText(e.target.value)} placeholder={"Buy groceries\nCall dentist\nFinish report\nReview PR #42\n..."} rows={8}
          style={{width:"100%",padding:"12px",background:"var(--inp,rgba(255,255,255,.04))",border:"1px solid rgba(77,150,255,.12)",borderRadius:8,color:"var(--tx,#d0d8e4)",fontSize:11,fontFamily:"inherit",resize:"vertical",boxSizing:"border-box",lineHeight:1.6}}/>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          {Object.entries(IMP).map(([k,v])=>{const act=bulkImp===k;return(
            <button key={k} onClick={()=>setBulkImp(k)} style={{padding:"4px 10px",fontSize:9,fontFamily:"inherit",cursor:"pointer",background:act?(v.color?v.color+"18":"rgba(255,255,255,.06)"):"var(--inp,rgba(255,255,255,.02))",border:`1px solid ${act?(v.color||"var(--brd3,rgba(255,255,255,.15))")+"44":"var(--brd,rgba(255,255,255,.04))"}`,borderRadius:5,color:act?(v.color||"var(--tx,#d0d8e4)"):"var(--tx3,#556)",fontWeight:act?700:400,transition:"all .1s"}}>{v.badge?v.badge+" ":""}{v.label}</button>);})}
        </div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          {DUR_PRESETS.map(d=>{const act=bulkDur===d.v;return(
            <button key={d.v} onClick={()=>setBulkDur(act?"":d.v)} style={{padding:"4px 8px",fontSize:8,fontFamily:"inherit",cursor:"pointer",background:act?"rgba(77,150,255,.12)":"var(--inp,rgba(255,255,255,.02))",border:`1px solid ${act?"rgba(77,150,255,.3)":"var(--brd,rgba(255,255,255,.04))"}`,borderRadius:4,color:act?"#7bb8ff":"var(--tx3,#556)",fontWeight:act?700:400,transition:"all .1s"}}>{d.l}</button>);})}
          {bulkDur&&<button onClick={()=>setBulkDur("")} style={{...tinyBtn,fontSize:8,color:"var(--tx2,#888)"}}>{"\u2715"}</button>}
        </div>
