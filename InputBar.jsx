...
export function DesktopInputBar({ input, setInput, addFish, actTank, newImp, setNewImp, newDur, setNewDur, newDue, setNewDue, setBulkModal }) {
  return(
    <div style={{padding:"8px 12px",background:"rgba(5,9,18,.96)",borderTop:"1px solid rgba(255,255,255,.03)",zIndex:15,flexShrink:0}}>
      <div style={{display:"flex",gap:8}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addFish()}
          ...
          style={{flex:1,padding:"9px 14px",background:"rgba(255,255,255,.035)",border:"1px solid rgba(255,255,255,.06)",borderRadius:7,color:"#d0d8e4",fontSize:14,fontFamily:"inherit"}}/>
        ...
        <button onClick={()=>setBulkModal(true)} title="Bulk add (paste list)" style={{padding:"9px 12px",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:7,color:"#8898aa",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{"\u2630"}</button>
      </div>
      <div style={{display:"flex",gap:5,marginTop:6,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <div style={{width:1,height:16,background:"rgba(255,255,255,.04)",margin:"0 3px"}}/>
        {DUR_PRESETS.filter((_,i)=>i%2===0||i<4).map(d=>{const act=newDur===d.v;return(
          <button key={d.v} onClick={()=>setNewDur(act?"":d.v)} style={{padding:"4px 8px",fontSize:11,fontFamily:"inherit",cursor:"pointer",background:act?"rgba(77,150,255,.12)":"rgba(255,255,255,.02)",border:`1px solid ${act?"rgba(77,150,255,.3)":"rgba(255,255,255,.04)"}`,borderRadius:5,color:act?"#7bb8ff":"#556",fontWeight:act?700:400,transition:"all .1s"}}>{d.l}</button>);})}
        {newDur&&<button onClick={()=>setNewDur("")} style={{...tinyBtn,fontSize:10,color:"#888"}}>{"\u2715"}</button>}
        <div style={{width:1,height:16,background:"rgba(255,255,255,.04)",margin:"0 3px"}}/>
        <input type="date" value={newDue} min={todayStr()} onChange={e=>setNewDue(e.target.value)}
          style={{padding:"3px 8px",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.06)",borderRadius:5,color:"#d0d8e4",fontSize:11,fontFamily:"inherit",colorScheme:"dark"}}/>
        {newDue&&<button onClick={()=>setNewDue("")} style={{...tinyBtn,fontSize:10,color:"#888"}}>{"\u2715"}</button>}
      </div>
    </div>);
}
